...
const double a = 2;
const unsigned nx = 200, nt = 200;
const double hx = (x1 - x0) / (nx - 1.);
const double ht = (t1 - t0) / (nt - 1.);
...
double calcLeftCorner(double u_mk, double u_m_1k, double f_mk) {
	return u_mk * (1 - ht / hx) + ht / hx * u_m_1k + ht * f_mk;
}
...
double solution(double x, double t) {
	if (x >= 2 * t) {
		...
		return x * t - 0.5 * t * t + 1 / 8 * (2 * t - x) * (2 * t - x) + exp(-t + 0.5 * x);
	}
}
int main(int argc, char* argv[])
{
	...
	fprintf(outputFile, "%g, %g, %g, %g, %g, %g\n", x0, x1, hx, t0, t1, ht);
	...
	for (int i = 0; i < nt; ++i) {
		for (int j = 0; j < nx; ++j) {
			fprintf(outputFile, "%g, ", solution(j * nx, i * nt));
		}
		fprintf(outputFile, "\n");
	}
